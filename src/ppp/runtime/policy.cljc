  ...
  {:version capability-version
   ...
   {:change
    ...
    ["A registered action handler receives the submitted payload map directly, never a {:params ...} wrapper."
     ...
     "In the Shared Public POC, product accounts are workspace-owned platform data reached through auth-register!, auth-login!, auth-logout!, auth-current-user, auth-require-user!, auth-change-password!, and auth-delete-account!. The typed boundary protects the shared host process; it does not make the generated product's auth schema or account state part of the Control Plane."
     ...
     "blob-put! accepts {:id optional-ascii-id :name :content-type :content-base64}; omit :id to let the Kernel return a new opaque UUID. Never derive an id from a user filename. Explicit ids must match [A-Za-z0-9][A-Za-z0-9_-]{0,95}; Unicode and punctuation belong in :name. blob-get and blob-delete! accept the returned id; blob-list takes no arguments. Objects are limited to 4 MiB and 64 per session by default."
    ...
    ["The page component argument contains only host context such as :session-id; product state lives in @api/page-state."
     ...
     "Use (api/action! :votes/create {:project-id id :voter-type voter-type} :items/data) in events; the third argument stores the successful response at that page-state key. A failure is always stored at [:runtime/action-errors target-key] and also at the target key when no prior result exists, so durable displayed data is not erased. Render the bounded {:error string :code keyword} failure instead of leaving an optimistic or loading state visible."
    ...
    {:server
     ...
     :resources
     "(defn process! [{:keys [id]}] (api/search-upsert! :objects id {:text (str id \" processed\") :metadata {:processed true}}) (api/publish! :objects/processed {:id id}) {:processed id})\n(defn upload! [{:keys [name content-type content-base64]}] (let [object (api/blob-put! {:name name :content-type content-type :content-base64 content-base64}) id (:id object)] (api/search-upsert! :objects id {:text name :metadata {:name name}}) (api/publish! :objects/changed {:id id}) {:object object}))\n(api/register-action! :objects/upload upload!)\n(api/register-job! :objects/process process!)"}}})
