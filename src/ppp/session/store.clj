...
(defn normalize-session-title
  [value]
  (let [title (when (string? value) (str/trim value))
        code-points (when title
                      (.codePointCount ^String title 0 (.length ^String title)))]
    (when (or (str/blank? title)
              (> (or code-points 0) session-title-limit)
              (re-find #"[\p{Cc}]" title))
      (throw (ex-info "Session title is invalid"
...
(defn create-session!
  ...
  ([^Store store {:keys [title] :or {title "Untitled product"}}]
   (let [title (normalize-session-title title)
